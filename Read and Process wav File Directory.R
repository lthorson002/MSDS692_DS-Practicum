...
setwd("D:/HeartBeatSounds/HB Data")
...
SpectrumAnalyzehb <- function(X, bp = c(0,22), wl = 2048, threshold = 5, parallel = 1){
  ...
  x <- as.data.frame(lapp(1:length(start), function(i) { 
    ...
    plot(r, ylim = c(-5000,5000),col=1)
    
    #75-1500 Bandpass noise filter
    r <- bwfilter(r, 22050, n = 3, from = 75, to = 1500, bandpass = TRUE, listen = FALSE, output = "Wave")
    
    #plot filtered waveform
    plot(r, ylim = c(-3000,3000),col=4)
    ...
    plot.ts(leftTS, ylim = c(-1, 1),xlim = c(0, 200000),col=5)
...
#Waveform Characterization - Seewave Package
    ...
    peakf <- 0#seewave::fpeaks(songspec, f = r@samp.rate, wl = wl, nmax = 2, plot = FALSE)[1, 1]
...
#modulation index calculation
    ...
    return(c(duration, meanfreq, sd, median, Q25, Q75, IQR, skew, kurt, sp.ent, sfm, mode, 
             centroid, peakf, meanfun, minfun, maxfun, meandom, mindom, maxdom, dfrange, modindx, m, H,ACI, zX, sh, bio_ind_L, ad_result_L))
  }))
  
  #Organize above results
  rownames(x) <- c("duration", "meanfreq", "sd", "median", "Q25", "Q75", "IQR", "skew", "kurt", "sp.ent", "sfm","mode", "centroid", "peakf", "meanfun", "minfun", "maxfun", "meandom", "mindom", "maxdom", "dfrange", "modindx", "m", "H", "ACI", "zX", "sh", "bio_ind_L", "ad_result_L")
...
beat <- function(filePath) {
  ...
  # Setup paths.
  currentPath <- getwd()
  fileName <- basename("D:/HeartBeatSounds/HB Data")
  path <- dirname("D:/HeartBeatSounds/HB Data")
  ...
  # Process files.
  acoustics <- SpectrumAnalyzehb(data, parallel=1)
...
# Facet boxplot of data comparing normal vs murmur
ggplot(hbeat2) +
